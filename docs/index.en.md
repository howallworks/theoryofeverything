...
According to [[GTR.en|GTR]], gravity is a deformation of space-time without energy transfer (it does not have a force effect), but the loss of energy to gravitational radiation ([gravitational waves](https://en.wikipedia.org/wiki/Gravitational_wave)) in close binary systems is also, according to the statement, in good conform with the GTR model. In fact, if we take any objects of the universe, then each pair of them is also a binary system, and therefore objects lose energy to gravitational radiation (gravitational waves). It's just that the speed of processes and the magnitude of gravitational radiation are not as large as in close binary systems. This means that even according to GTR ***all objects of the universe (wich having mass) generate gravitational radiation, spend energy on it and exert a force effect on surrounding objects***.
