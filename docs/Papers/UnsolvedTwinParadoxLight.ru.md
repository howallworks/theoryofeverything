...
Необходимо также упомянуть эксперимент с тремя или более поляризаторами повернутыми на разные углы, который интерпретируют как неопровержимое доказательство квантово-механической природы вселенной. Если поставить два поляризационных фильтра под углом девяносто градусов то считается что свет не проходит так как первый блокирует одно направление волны, а второй перпендикуляное, то есть все остальное. И из-за этого и не проходят фотоны. Если взять три поляризатора $0^o$, $45^o$, $90^o$, то свет частично проходит. Почему? Дело в неправильной интерпретации фотона и механизма действия поляризатора. Если фотон проходит поляризатор, то он переизлучается с особым распределением плоскости поляризации относительно оси данного поляризатора, а именно: $cos^2(2x)$, где  $-\pi/4 \leq x \leq \pi/4$. И поляризатор пропускает только фотоны с плоскостью поляризации в интервале $-\pi/4 \leq x \leq \pi/4$ относительно оси данного поляризатора. Получаем, что не важно сколько поляризаторов фотон проходил до этого. Важно что каждый фотон будет заново переизлучен при прохождении сквозь поляризатор и изменит свою плоскость поляризации. Получается чем больше поляризаторов отклоненных на минимальные углы мы поставим между поляризатором $0^o$ и поляризатором $90^o$, тем больше фотонов пройдет. Это объясняет все наблюдаемые экспериментальные данные и эффекты.
