...
Последняя Pdf версия Теории всего: [Теория всего.ru(2023.02.14).pdf](Files/Теория_всего.ru(2023.02.14).pdf)
...
Фактически ученые предполагают наличие [[Accretion disk.ru|аккреционного диска]], который используя энергию [[SMBH.ru|СМЧД]] каким-то образом формирует релятивистскую струю. И это сейчас основное объяснение происходящего с [[Blazar.ru|блазаром]]. 
...
Рассмотрим аннигиляцию протона и антипротона $p+ \overline p$ (мне больше нравится запись: $p^++p^-$):
...
**Рисунок 2 – Эффект запаздывания движущихся часов по сравнению с покоящимися в НСО** 
...
Рассмотрим эксперимент №1 согласно общей теории относительности, представленный на рисунке 4, относительно объекта 1 (о1) – $СО_{о1}$ (рисунок 4.1) и относительно объекта 2 (о2) – $СО_{о2}$ (рисунок 4.2).
...
Теперь рассмотрим другой эксперимент, представленный на рисунке 5, согласно общей теории относительности. Сразу нужно отметить, что время ускорений объектов много меньше времени движения объектов с постоянными скоростями (это условие необходимо для исключения спекуляций о влиянии ускорений объектов на результаты). В фазе 1 часы всех наблюдателей синхронизированы (показывают одно и тоже время). **dt** – промежуток времени, проведенный объектами в данной фазе по часам объекта с которым связана система отсчета.

![[pic4.png]] 

**Рисунок 5 – Эксперимент №2**
...
#### Парадокс близнецов. Дополнительные варианты экспериментов

После того как Эйнштейн сформулировал специальную теорию относительности (СТО) придумали мысленный эксперимент, в котором не понятно какой объект испытывает "замедление времени" - Парадокс близнецов. Однако, хотя это и не следует из специальной теории оносительности, затем ученые считающие корректной СТО указали, что никакого парадокса нет, так как испытывать замедление времени будет близнец испытывающий ускорение (хотя это также не позволяет полностью решить случай эксперимента №2). 

Приведем мысленный эксперимент №3.
Два близнеца летят в пристыкованных друг другу ракетах. Первый и второй садится в свои ракеты, ускоряются с одинаковым ускорением в разные стороны, летят длительное время, тормозят с одинаковым ускорением, ускоряются в обратном направлении с одинаковым ускорением, летят длительное время, тормозят с одинаковым ускорением, стыкуются и встречяются в ракете. 

А теперь предсказания моей теории всего. 

Вы думаете, что в эксперименте №3 оба близнеца будут иметь одинаковый возраст?
Только в одном случае из всех. Когда начальная скорость ракет относительно НСО или вакуума или относительно реликтового излучения была равна нулю. Для всех остальных случаев (была начальная скорость отностиельно вакуума или относительно реликтового излучения) близнецы будут иметь разный возраст (пройдет разное количество периодов всех процессов в их телах).

Моложе будет тот близнец, скорость которого была ближе и дольше к скорости света относительно вакуума или относительно реликтового излучения. Фактически формула замедления времени корректно применима только относительно НСО/вакуума/реликтового излучения/с погрешностью относительно Земли. Особо подчеркну, что хоть формула и применима, но она не отражает физику процесса, так как замедляется не время, а увеличивается длительность всех процессов в объекте.

Эксперимент №3 с конкретным вариантом значений скоростей. 
Два близнеца летят в пристыкованных друг другу ракетах (0.2c относительно НСО/вакуума/реликтового излучения/**с погрешностью** относительно Земли). Первый и второй садится в свои ракеты, ускоряются с одинаковым ускорением в разные стороны , летят длительное время (~ c, ~ -0.6c), тормозят с одинаковым ускорением (0.2c), ускоряются в обратном направлении с одинаковым ускорением, летят длительное время (0.19c, 0.21c), тормозят с одинаковым ускорением (0.2c), стыкуются и встречяются в ракете. 
В итоге для данного конкретного случая первый близнец будет моложе второго. Однако для других случаев будут другие варианты соотношений возрастов. 

Такого рода эксперименты не сложно провести. Один из простейших вариантов - использовать короткоживущие частицы, которые быстро распадаются при скоростях значительно меньших, чем скорость света. Другой гораздо более сложный, но наглядный вариант это все же использовать ракеты с атомными часами. 

Самый простой способ проверки взять две короткоживущие частицы полученные в одно и тоже время и провести два варианта экспериментов.

Классический Парадокс близнецов будет выглядеть так. 

Вариант №1 - разогнанные до одинаковой скорости близкой к нулю относительно НСО/вакуума/реликтового излучения/Земли. С точки зрения каждой из них они покоятся. 
Отметим этот момент как начало эксперимента. Ускорим первую частицу таким образом чтобы она двигалась быстрее относительно НСО/вакуума/реликтового излучения/Земли (желательно почти до скорости света), подождем значительное время $t$, а затем ускорим в обратном направлении до изначальной скорости (около 0). В результате вторая(покоящаяся/двигающаяся без ускорений) частица распадется быстрее первой.

Обратный вариант Парадокса близнецов будет выглядеть так.

Вариант №2 - разогнанные до одинаковой скорости близкой к скорости света относительно НСО/вакуума/реликтового излучения/Земли. С точки зрения каждой из них они покоятся. 
Отметим этот момент как начало эксперимента. Ускорим первую частицу таким образом чтобы она двигалась медленне относительно НСО/вакуума/реликтового излучения/Земли (желательно почти до нулевой скорости), подождем значительное время $t$, а затем ускорим в обратном направлении до изначальной скорости (близкой к скорости света. Такой же как и у второй частицы). В результате первая частица распадется быстрее второй. Хотя с точки зрения специальной теории относительности (дополненной пониманием кто ускорялся, а кто нет) должна распасться сначала вторая(покоящаяся/двигающаяся без ускорений), а затем первая. 

Можно проводить этот эксперимент с ракетами, но это будет сложнее реализовать.

Фактически вариант №2 это тот же самый эксперимент из Парадокса близнецов. Но если там начальная скорость относительно НСО/вакуума/реликтового излучения/Земли была близкой к нулю, то тут она будет близкой к скорости света. С точки зрения теории относительности это ничего не меняет, так как нам важна именно их скорость относительно друг друга и понимание кто из них ускорялся, а кто нет.

Можно придумать ещё множество отличных вариантов для проверки. 

***Вот один из лучших экспериментов.***

***Возьмем две коротко живущие частицы полученные в одно и тоже время, пусть их начальная скорость $v$ относительно НСО/вакуума/реликтового излучения/Земли и отметим начало эксперимента.*** 

***Они летят вместе. Пусть $v=0$. Ускорим их до $0.5c$. Первую ускорим до скорости близкой к скорости света, а вторую до 0. Подождем значительное время $t$. Ускорим обе обратно до $0.5c$. Ускорим их обе до 0. Ускорим их обе до $-0.5c$. Первую ускорим до скорости близкой к $-c$, а вторую до 0. Подождем значительное время $t$. Ускорим их обе до $-0.5c$. Ускорим их обе до 0. Они летят вместе.***

***С точки зрения каждой из частиц(можно и с кораблями проверять) они двигались и ускорялись одинаково относительно друг друга и в этом то и заключается истиный вариант Парадокса близнецов. С точки зрения каждой из частиц другая двигалась быстрее, а ускорения были одинаковы. И тут уже специальная теория относительности бессильна без введения сторонних наблюдателей даже с учетом ускорений. Более того если сторонний наблюдатель будет двигаться со скоростью близкой к скорости света относительно НСО/вакуума/реликтового излучения/Земли, то он сделает ошибочное предположение о том какая частица распадется первой.*** 
...
***Они летят вместе. Пусть $v=0.5c$. Первую ускорим до 0, а вторую до $c$. Подождем значительное время $t$. Ускорим обе обратно до $0.5c$. Первую ускорим до $0.6c$, а вторую до $0.4c$. Подождем значительное время $5t$. Ускорим их обе до $0.5c$. Они летят вместе. Для каждого из двух объектов этого случая с точки зрения теории относительности скорости и ускорения были одинаковыми, а значит должно пройти одинаковое количество времени.*** 
...
***Они летят вместе. Пусть $v=-0.5c$. Первую ускорим до $-c$, а вторую до 0. Подождем значительное время $t$. Ускорим обе обратно до $-0.5c$. Первую ускорим до $-0.4c$, а вторую до $-0.6c$. Подождем значительное время $5t$. Ускорим их обе до $-0.5c$. Они летят вместе. Для каждого из двух объектов этого случая с точки зрения теории относительности скорости и ускорения были одинаковыми, а значит должно пройти одинаковое количество времени.*** 
...
***Для последних двух случаев с объектами происходило абсолютно одно и тоже, но из-за разной начальной скорости относительно НСО/вакуума/реликтового излучения/Земли получились противоположные результаты. Однако с точки зрения теории относительности разницы абсолютно никакой.***

С другими начальными скоростями относительно НСО/вакуума/реликтового излучения/Земли и вариациями ускорений будут получаться другие варианты результатов, но суть таже - теория относительности Эйнштейна не может корректно решить Парадокс близнецов.

На самом деле тут дело не в ускорениях, а в скорости относительно вакуума или относительно реликтового излучения. Те объекты, которые будут двигаться со скоростью ближе к скорости света относительно НСО или вакуума или относительно реликтового излучения - будут "жить" дольше, но не из-за "замедления времени", а из-за увеличения длительности всех процессов в объекте. Подробности увеличения длительности всех процессов в объекте при скоростях близких к скорости света были рассмотрены ранее, например, в разделе "Эффект запаздывания движущихся часов по сравнению с покоящимися в НСО".

В итоге мы снова можем видеть и проверить экспериментальное несоответствие специальной теории относительности Эйнштейна реальности. Опять же повторюсь, что вся проблема в термине секунда, который зависит от скорости движения относительно НСО/вакуума/реликтового излучения/Земли. Детали были приведены в разделе "Как дать определение термину «Время», чтобы создать теорию относительности".
...
Все объекты Вселенной (обладающие массой) генерируют гравитационное излучение, тратят на это энергию и оказывают силовое воздействие на окружающие объекты (детали в разделе «[[theoryofeverything.ru#Неравномерность гравитации. Квант гравитон. Блазар - СМЧД с релятивистскими струями|Неравномерность гравитации. Квант гравитон. Блазар - СМЧД с релятивистскими струями]]»)

Возьмём все частицы Стандартной модели, кроме фотона и, возможно, глюона. Все они излучают [[Quantum graviton.ru|квант гравитоны]], то есть обладают активной массой. Следовательно они нестабильны, так как запас [[Quantum graviton.ru|квант гравитонов]] ограничен в изолированной среде или в далекой перспективе при разлете материи/антиматерии во Вселенной. 
...
[[Photon.ru|Фотон]] вообще состоит исключительно из группы $к.ф.^+$, либо исключительно из группы $к.ф.^-$. Прохождение поляризатора это тоже воздействие на фотон (группу квант фотонов) и переизлучение, что по сути имеет место для всего безвакуумного пространства.
...
-   элементарные частицы возникли из ниоткуда в очень малом объеме трехмерного пространства Вселенной. После чего произошел Большой взрыв ставший отражением динамики элементарных частиц;
-   более 27 миллиардов лет прошло времени с момента начала Большого взрыва;
...
- период излучения атома цезия-133 зависит от скорости движения относительно НСО или ИСО. Вообще любые процессы для систем элементарных частиц зависят от скорости движения относительно НСО (от времени жизни частиц до атомов и макрообъектов). Здесь особенность заключается в определении термина "Время" отличного от того, которое стало основой [[GTR.ru|ОТО]] 
